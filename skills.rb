...
class << Skills
  def manifest(skill, user, time,entities,map,canvas,minibuffer)
    ...
    when :six_realms
      ...
      targets = get_entity_seen(user,entities,map).select{|e| e.pos.x == user.pos.x || e.pos.y == user.pos.y}.take(1)
      targets.delete(user)
    ...
    when :magic_dust
      ...
      if single_target.empty?
        ...
        t = single_target.first
        a.delete t
        targets = a.select{|e| entity_distance(t,e) <= 4}
        targets << t
      end
    when :flower_slash
    ...
    when :ambush
      ...
      possible_ambush_places.select{|p| movable?(entities,map,p[0],p[1])}.sample(4).each do |p|
